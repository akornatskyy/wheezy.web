...
class SignInHandler(MembershipBaseHandler, RedirectQueryStringReturnPathMixin):
    ...
    @attribute
    def model(self):
        return attrdict({"remember_me": False})
    ...
    def get(self, credential=None):
        ...
        return self.render_response(
            "membership/signin.html", credential=credential, model=self.model
        )
    ...
    def post(self):
        ...
        if (
            not self.try_update_model(credential)
    ...
    def authenticate(self, credential):
        ...
        self.principal = Principal(
            id=credential.username, roles=roles, alias=credential.username
        )
...
class SignUpHandler(MembershipBaseHandler):
    ...
    def get(self, registration=None):
        ...
        return self.render_response(
            ...
            questions=questions,
            account_types=tuple(
                (k, self.gettext(v)) for k, v in account_types
            ),
        )
    ...
    def post(self):
        if not self.validate_resubmission():
            self.error(
                self._(
                    "Your registration request has been queued. "
        ...
        registration = Registration()
        if (
            not self.try_update_model(self.model)
